...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      stars = []
      for (let i = 0; i < 380; i++) {
        stars.push({ x: Math.random(), y: Math.random(), r: rnd(0.15,1.8), a: rnd(0.3,1), ts: rnd(0.004,0.018), to: Math.random()*Math.PI*2, col: Math.random()<0.3?'#a0d8ff':Math.random()<0.5?'#00ffcc':'#ffffff', par: rnd(0.06,0.28) })
      }
      nebulas = [
    ...
    function draw() {
      ...
      bg.addColorStop(0,'#00030f'); bg.addColorStop(0.45,'#020520'); bg.addColorStop(1,'#050212')
      ctx.fillStyle=bg; ctx.fillRect(0,0,W,H)
      const sOff = scrollY*0.3
      nebulas.forEach(n=>{
        const nx=n.x*W, ny=n.y*H-sOff*n.par*3, nr=n.r*Math.min(W,H)
        const g=ctx.createRadialGradient(nx,ny,0,nx,ny,nr)
        g.addColorStop(0,`rgba(${n.c},${n.i})`); g.addColorStop(0.4,`rgba(${n.c},${n.i*0.35})`); g.addColorStop(1,`rgba(${n.c},0)`)
        ctx.fillStyle=g; ctx.fillRect(0,0,W,H)
      })
      stars.forEach(s=>{
        const twinkle=Math.sin(frame*s.ts+s.to)*0.3+0.7
        const sy=((s.y*H*3-sOff*s.par)%(H*3)+H*3)%(H*3)%H
        ctx.beginPath(); ctx.arc(s.x*W,sy,s.r,0,Math.PI*2)
        ctx.fillStyle=s.col; ctx.globalAlpha=s.a*twinkle; ctx.fill()
        if(s.r>1.2){ctx.beginPath();ctx.arc(s.x*W,sy,s.r*3.5,0,Math.PI*2);ctx.globalAlpha=s.a*twinkle*0.18;ctx.fill()}
        ctx.globalAlpha=1
      })
      if(Math.random()<0.012){shooters.push({x:Math.random()*W,y:Math.random()*H*0.35,len:rnd(90,170),spd:rnd(9,18),a:1,ang:rnd(0.28,0.68)})}
      ...
      shooters.forEach(s=>{
        ...
        g.addColorStop(0,'rgba(0,255,200,0.95)'); g.addColorStop(1,'rgba(0,255,200,0)')
        ctx.strokeStyle=g; ctx.lineWidth=1.2; ctx.beginPath(); ctx.moveTo(s.x,s.y)
        ctx.lineTo(s.x-Math.cos(s.ang)*s.len,s.y-Math.sin(s.ang)*s.len); ctx.stroke(); ctx.restore()
        s.x+=Math.cos(s.ang)*s.spd; s.y+=Math.sin(s.ang)*s.spd; s.a*=0.93
      })
  ...
  function scrollToSection(id){const el=document.querySelector(id);if(!el)return;window.scrollTo({top:el.getBoundingClientRect().top+window.scrollY-90,behavior:'smooth'})}
  function toggleFaq(btn){const item=btn.closest('.faq-item');const isOpen=item.classList.contains('open');item.closest('.faq-block').querySelectorAll('.faq-item.open').forEach(el=>el.classList.remove('open'));if(!isOpen)item.classList.add('open')}
  ...
  const faqData={
    kimiz:['EchoKush nasıl bir platform?','Kimler başvurabilir?','Platform ücretsiz mi?','EchoKush Vakfı nedir?','İletişim ve destek nasıl sağlanır?'],
    yayincilar:['Podcast nedir, nasıl üretilir?','Sesli kitap üretmek için ne gerekli?','Yayıncı olarak hangi içeriklere erişebilirim?','Eser sahibiyle nasıl iletişim kurarım?','Yayıncı başvurusu nasıl değerlendirilir?'],
    eserler:['Hangi eser türleri kabul edilir?','Telif haklarım güvende mi?','Eserim hangi formatlara dönüştürülür?','Yayıncıyı ben mi seçiyorum?','Eser başvurusu nasıl yapılır?'],
    gelir:['Ödemeler nasıl gerçekleşir?','Gelir payı oranları nasıl belirleniyor?','Minimum ödeme eşiği nedir?','Vergi ve fatura süreci nasıl işler?',"EchoKush'un platform kesintisi ne kadar?"],
    isleyis:['Hakem kurulu kimlerden oluşur?','Başvurum ne kadar sürede değerlendirilir?','Anlaşmazlık durumunda ne olur?','Sözleşme süreci nasıl işler?',"Dashboard'da neler takip edebilirim?"],
  }
    ...
    <div className="faq-block">
      {items.map((q,i)=>(
        <div className="faq-item" key={i}>
          ...
          <div className="faq-answer"><div className="faq-answer-inner">Cevap buraya gelecek.</div></div>
        </div>
      ))}
    </div>
  )
  ...
  return(
    <>
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}body{background:#00030f;font-family:Georgia,serif;overflow-x:hidden;color:#fff}
        ...
        .hero-sub{font-size:clamp(26px,3.8vw,46px);font-weight:700;color:#fff;letter-spacing:1px;margin-bottom:28px;opacity:0;animation:fadeUp 1s 0.6s forwards;line-height:1.25}
        ...
        .section.img-left{flex-direction:row}.section.img-right{flex-direction:row-reverse}
        ...
        .visual-placeholder{text-align:center}.visual-placeholder .icon{font-size:48px;margin-bottom:14px}.visual-placeholder p{font-size:9px;letter-spacing:3px;text-transform:uppercase;font-family:sans-serif;color:rgba(0,255,180,0.2)}
        ...
        .faq-question-text{font-size:13px;font-family:sans-serif;font-weight:600;color:rgba(200,220,255,0.8);line-height:1.5}
        .faq-icon{width:22px;height:22px;border-radius:50%;border:1px solid rgba(0,255,180,0.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px;color:#00ffb4;transition:transform 0.3s,background 0.3s}
        ...
        .modal-close{position:absolute;top:18px;right:22px;font-size:18px;color:rgba(0,255,180,0.4);cursor:pointer;background:none;border:none;font-family:sans-serif}
        ...
        .modal-btn{padding:13px 40px;background:#00ffb4;color:#00030f;border:none;font-size:11px;letter-spacing:3px;text-transform:uppercase;font-family:sans-serif;font-weight:700;border-radius:3px;cursor:pointer}
      ...
      <div className="page">
        ...
        <section id="hero">
          ...
          <div className="scroll-hint"><span>Keşfet</span><div className="scroll-line"/></div>
        </section>
        ...
        <section className="section img-left" id="kimiz">
          ...
          <div className="section-text">
            ...
            <p className="section-body">EchoKush; yazarlar, şairler ve düşünce insanlarının ürettikleri eserlerin çok daha geniş kitlelere ulaşmasını sağlamak için kurulmuş bir gelir ortaklığı ve içerik dönüşüm platformudur.<br/><br/>Eser sahiplerini yaratıcı yayıncılarla buluşturur.</p>
        ...
        <section className="section img-right" id="yayincilar">
          ...
          <div className="section-text">
            ...
            <p className="section-body">Sesli kitap yapımcıları, müzik destekli anlatım sanatçıları, podcast üreticileri ve video yaratıcıları için EchoKush; lisanslı, özgün ve değerli içeriklere erişim kapısıdır.<br/><br/>Her dinlenme, her izlenme sana ve eser sahibine adil bir gelir akışı sağlar.</p>
        ...
        <section className="section img-left" id="eserler">
          ...
          <div className="section-text">
            ...
            <p className="section-body">Kitabın, şiirin, makalenin yalnızca raflarda kalmak zorunda değil. Telif haklarını güvende tut, yayıncılarla şeffaf sözleşmelerle çalış.</p>
        ...
        <section className="section img-right" id="gelir">
          ...
          <div className="section-text">
            ...
            <ul className="gelir-list">
              <li><span className="dot"></span>Gelir payı şeffaf sözleşmeyle belirlenir.</li>
              <li><span className="dot"></span>Her dinlenme ve izlenmeden elde edilen gelir anlık takip edilir.</li>
              <li><span className="dot"></span>Platform payı asgari düzeyde tutulur.</li>
              <li><span className="dot"></span>Ödemeler aylık döngüde otomatik gerçekleşir.</li>
              <li><span className="dot"></span>Anlaşmazlıklarda bağımsız hakemlik devreye girer.</li>
            </ul>
        ...
        <section className="section img-left" id="isleyis">
          ...
          <div className="section-text">
            ...
            <p className="section-body">Her eser başvurusu uzman hakem kurulunun incelemesinden geçer. Onaylanan yayıncılar eser havuzuna erişim kazanır ve eser sahipleriyle doğrudan iletişime geçebilir.</p>
        ...
        <section id="apply">
          ...
          <p className="apply-desc">Eserini sesle buluşturmak ya da sesi esere dönüştürmek için başvurunu yap.</p>
        ...
        <footer>
          ...
          <p className="f-copy">EchoKush Vakfı yakında</p>
        </footer>
      </div>
      ...
      <div className="modal-overlay" id="dashModal">
        <div className="modal-box">
          ...
          <p className="modal-desc">Dashboard geliştirme aşamasındadır. Yakında hizmetinizde olacak.</p>
